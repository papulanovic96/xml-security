...
public class ReservationEndpoint {
    ...
    @ResponsePayload
    public CudReservationResponse create(@RequestPayload CreateReservationRequest request) {
        return new CudReservationResponse(reservationService.create(request));
    }
	
	@PayloadRoot(namespace = NAMESPACE_URI, localPart = "updateReservationRequest")
    @ResponsePayload
    public CudReservationResponse update(@RequestPayload UpdateReservationRequest request) {		
